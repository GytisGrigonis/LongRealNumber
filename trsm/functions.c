...
int checkIfNumber(const char *string)
{
    int index = 0;
...
void insertBegin(Big_int **head, int temp)
{
    Big_int *elem;
...
int division(Big_int *head1, Big_int *head2, Big_int **finalAnswer)
{
    Big_int *temp, *one;
    if(head1 == NULL || head2 == NULL) return 1;
    if(head2 -> data == 0 && head2 -> forw == NULL) return 2;
    one = (Big_int*)malloc(sizeof(Big_int));
    one -> data = 1;
    one -> forw = NULL;
    one -> backw = NULL;
...
bigInt createBigInt(int *error, char *number)
{
    ...
    while(number[index] != '\0'){
        ...
        elem -> backw = prev;
        elem -> forw = NULL;
        elem -> data = (int)number[index] - 48;
        index ++;
    }
    *error = 0;
    return head;
}
void bigAdd(bigInt head1, bigInt head2, bigInt *answer, int *error)
{
    if((*answer) != NULL)
        deleteList(answer);
    *error = sum(head1, head2, answer);
}
void bigSubtract(bigInt head1, bigInt head2, bigInt *answer, int *error)
{
    if((*answer) != NULL)
        deleteList(answer);
    *error = subtraction(head1, head2, answer);
}
void bigMultiply(bigInt head1, bigInt head2, bigInt *answer, int *error)
{
    if((*answer) != NULL)
        deleteList(answer);
    *error = multiplication(head1, head2, answer);
}
void bigDivide(bigInt head1, bigInt head2, bigInt *answer, int *error)
{
    if((*answer) != NULL)
        deleteList(answer);
